...
//      ES_Timer_InitTimer(SERVICE0_TIMER, HALF_SEC);
    ...
    {
//      ES_Timer_InitTimer(SERVICE0_TIMER, FIVE_SEC*4);
    ...
    /*case ES_NEW_KEY:   // announce
    {
      ...
      {
        NewEvent.EventType = EV_SCORE_UPDATE;
      }
      if ('x' == ThisEvent.EventParam)
      {
        printf("Status is %d\n\r", GetGameStatus());
        uint8_t Poss = GetPossession();
        if (Poss == 1) {
          printf("Red possession\n\r");
        } else if (Poss == 2) {
          printf("Blue possession\n\r");
        } else {
          printf("No possession\n\r");
        }
        printf("Shot clock is %d\n\r", GetShotClock());
      }
      if ('b' == ThisEvent.EventParam)
      {
        NewEvent.EventType = ES_TIMEOUT;
        NewEvent.EventParam = HANDSHAKE_TIMER;
        SetNumBalls(GetNumBalls() + 10);
      }
      if ('o' == ThisEvent.EventParam)
      {
        NewEvent.EventType = EV_OBJECT_DETECTED_RETRO;
      }
      if ('u' == ThisEvent.EventParam)
      {
        NewEvent.EventType = EV_OPEN_SHOT_DETECTED;
      }
      if ('a' == ThisEvent.EventParam)
      {
        NewEvent.EventType = EV_OBJECT_DETECTED_SHARP;
      }
      if ('l' == ThisEvent.EventParam)
      {
        NewEvent.EventType = EV_LINE_HIT;
      }
      if ('s' == ThisEvent.EventParam)
      {
        NewEvent.EventType = EV_SWITCH_HIT;
      ...
      {
        NewEvent.EventType = EV_ATTACK_GOAL_DETECTED;
      }
      if ('r' == ThisEvent.EventParam)
      {
        NewEvent.EventType = EV_RELOADER_DETECTED;
      }
      if ('d' == ThisEvent.EventParam)
      {
        NewEvent.EventType = EV_EARLY_DEFENSE;
      }
